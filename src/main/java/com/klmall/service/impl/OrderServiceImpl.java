...
import com.klmall.dao.*;
import com.klmall.pojo.*;
...
import com.klmall.vo.OrderItemVo;
import com.klmall.vo.OrderProductVo;
import com.klmall.vo.OrderVo;
import com.klmall.vo.ShippingVo;
...
public class OrderServiceImpl implements IOrderService {
    ...
    public ServerResponse cancelOrder(Integer userId, Long orderNo){
        ...
        if (order.getStatus() != Const.OrderStatusEnum.NO_PAY.getCode()){
            return ServerResponse.createByErrorMessage("已支付无法取消");
        }
    ...
    public ServerResponse createOrder(Integer userId, Integer shippingId){
        List<Cart> cartList = cartMapper.selectCheckedCartByUserId(userId);
        ServerResponse<List<OrderItem>> serverResponse = this.getCartOrderItems(userId, cartList);
        if (!serverResponse.isSuccess()){
            return serverResponse;
        }
        List<OrderItem> orderItemList = (List<OrderItem>)serverResponse.getData();
    ...
    private OrderVo assembleOrderVo(Order order, List<OrderItem> orderItemList){
        ...
        orderVo.setPaymentTypeDesc(Const.PayTypeEnum.codeOf(order.getPaymentType()).getValue());
    ...
    private Order assembleOrder(Integer userId, Integer shippingId, BigDecimal orderTotal){
        Order order = new Order();
        long orderNo = this.generateOrderNo();
        order.setOrderNo(orderNo);
        order.setStatus(Const.OrderStatusEnum.NO_PAY.getCode());
        order.setPostage(0);
        order.setPaymentType(Const.PayTypeEnum.ONLINE_PAY.getCode());
    ...
    private ServerResponse<List<OrderItem>> getCartOrderItems(Integer userId, List<Cart> cartList){
        ...
        for (Cart cartItem : cartList){
            ...
            if (Const.ProductStatusEnum.ON_SALE.getCode() != product.getStatus()){
                return ServerResponse.createByErrorMessage("产品【"+product.getName()+"】状态异常，不能售卖");
            }
    ...
    public ServerResponse<Map<String, String>> pay(Long orderNo, Integer userId, String path){
        ...
        String subject = new StringBuilder().append("klmall扫码支付，订单号：").append(outTradeNo).toString();
        ...
        String body = new StringBuilder().append("订单").append(outTradeNo).append(outTradeNo).append("金额").append(totalAmount).append("元").toString();
    ...
    public ServerResponse<String> sendGoods(Long orderNo){
        ...
        return ServerResponse.createBySuccessMessage("订单已发货");
    }
}
